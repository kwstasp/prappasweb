...
class Functions{
    ...
    public function set_css_style($post_id, $shortcode = false){
        if( $post_id ){
            ...
            if (isset($_GET['et_fb']) || (isset($_GET['action']) && $_GET['action'] =='elementor') || $shortcode) {
                return '<style type="text/css">'.get_post_meta($post_id, '_ultp_css', true).'</style>';
            } else {
    ...
    public function get_query($attr) {
        ...
        if ($this->is_builder($builder)) {
            ...
            //     $_include = (substr($attr['queryInclude'], 0, 1) === "[") ? json_decode($attr['queryInclude']) : explode(',', $attr['queryInclude']);
            //     if (is_array($_include) && count($_include)) {
            //         $archive_query['post__in'] = $_include;
